...
<div class="container">
    ...
    <div class="container" id="container_maincard">
        <div class="row">
            ...
            <a href="natecaj/{{ $item['id'] }}">
                <div class="card text-center">
                    @if (isset($item['attributes']['Naslovnica']['data']['attributes']['url']))
                    <img src="{{ config('likusConfig.likus_api_urlMain') }}{{ $item['attributes']['Naslovnica']['data']['attributes']['url'] }}" class="card-img-top mx-auto d-block mt-3 card-img" style="height:300px !important">
                    @else
                    ...
                    <div class="card-body">
                        <h5 class="card-title">
                            <a href="knjiga/{{ $item['id'] }}">
                                {{ $item['attributes']['Ime'] }}<br />
                                <a class="btn btn-secondary" style="color: white" href="{{ config('likusConfig.likus_api_urlMain') }}{{$item['attributes']['Pdf_natecaja']['data']['attributes']['url']}}">Poglej natečaj</a>
                            </a>
                        </h5>
                    </div>
                </div>
...
function generateMemberCards(responseData) {
  ...
  members.forEach(function(member) {
    ...
    if (
      ...
      profileImageUrl = "{{ config('likusConfig.likus_api_urlMain') }}" + item.Naslovnica.data.attributes.url;
    }
    ...
    cardTitleLink.innerHTML = item.Ime+`<br /><a href='{{ config('likusConfig.likus_api_urlMain') }}${item.Pdf_natecaja.data.attributes.url}'>Poglej natečaj</a>`;
...
function fetchMembers() {
  ...
  if (search && selectType) {
    fetch(`{{ config('likusConfig.likus_api_url') }}/natecaji?filters[${selectType}][$contains]=${search}&populate=*`)
      .then(response => {
        if (!response.ok) {
