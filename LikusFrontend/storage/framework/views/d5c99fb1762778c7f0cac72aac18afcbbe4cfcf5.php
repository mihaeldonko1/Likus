...
<div class="container">
        <div class="form-control" style="position: relative;">
            <div class="input-group" style="display: flex; align-items: stretch;">
                ...
                <select class="custom-select" id="selectType" style="border: 1px solid #ccc; border-left: none; padding: 10px; width: auto; outline: none;">
                    ...
                    <option value="Ime">Imenu natecaja</option>
                </select>
    ...
    <div class="container" id="container_maincard">
        <div class="row">
            ...
            <a href="natecaj/<?php echo e($item['id']); ?>">
                <div class="card text-center">
                    <?php if(isset($item['attributes']['Naslovnica']['data']['attributes']['url'])): ?>
                    <img src="<?php echo e(config('likusConfig.likus_api_urlMain')); ?><?php echo e($item['attributes']['Naslovnica']['data']['attributes']['url']); ?>" class="card-img-top mx-auto d-block mt-3 card-img" style="height:300px !important">
                    <?php else: ?>
                    ...
                    <div class="card-body">
                        <h5 class="card-title">
                            <a href="knjiga/<?php echo e($item['id']); ?>">
                                <?php echo e($item['attributes']['Ime']); ?><br />
                                <a class="btn btn-secondary" style="color: white" href="<?php echo e(config('likusConfig.likus_api_urlMain')); ?><?php echo e($item['attributes']['Pdf_natecaja']['data']['attributes']['url']); ?>">Poglej natečaj</a>
                            </a>
                        </h5>
                    </div>
                </div>
...
function generateMemberCards(responseData) {
  ...
  members.forEach(function(member) {
    ...
    card.onclick = function() {
      location.href = "natecaj/" + member.id;
    };
    ...
    if (
      item.Naslovnica &&
      item.Naslovnica.data &&
      item.Naslovnica.data.attributes &&
      item.Naslovnica.data.attributes.url
    ) {
      profileImageUrl = "<?php echo e(config('likusConfig.likus_api_urlMain')); ?>" + item.Naslovnica.data.attributes.url;
    }
    ...
    cardTitleLink.innerHTML = item.Ime+`<br /><a href='<?php echo e(config('likusConfig.likus_api_urlMain')); ?>${item.Pdf_natecaja.data.attributes.url}'>Poglej natečaj</a>`;
...
function fetchMembers() {
  ...
  if (search && selectType) {
    fetch(`<?php echo e(config('likusConfig.likus_api_url')); ?>/natecaji?filters[${selectType}][$contains]=${search}&populate=*`)
      .then(response => {
        if (!response.ok) {
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\mihad\OneDrive\Namizje\Praktikum3\Likus\LikusFrontend\resources\views/natecaji.blade.php ENDPATH**/ ?>